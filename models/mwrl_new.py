...
5. Alpha parameter to control MRL (CE) loss weight
"""
...
class MWRLLoss(nn.Module):
    ...
    Loss = alpha * sum_i w_i * CE_i + lambda * sum_i W(emb_i, emb_{i+1})
    
    where:
    - CE_i is cross-entropy loss at dimension i
    - W is the Wasserstein distance with custom cost
    - w_i are per-dimension weights
    - alpha is the MRL (CE) loss weight (set to 0 for Wasserstein-only training)
    ...
    def __init__(
        ...
        Args:
            ...
            alpha: Weight for MRL (CE) loss (default: 1.0, set to 0 for Wasserstein-only)
    ...
    def forward(
        ...
        # Compute CE losses (even if alpha=0, we compute for logging purposes)
        ce_losses = []
        ...
        # Total loss with alpha parameter
        total_loss = self.alpha * weighted_ce_loss + self.wasserstein_weight * wasserstein_loss
...
def create_mwrl_model(
    ...
    Args:
        ...
        pretrained: Use pretrained ResNet50 weights
        ce_weights: Optional weights for each CE loss
        alpha: Weight for MRL (CE) loss (default: 1.0, set to 0 for Wasserstein-only training)
        wasserstein_weight: Weight for Wasserstein regularization (lambda)
        beta: Beta parameter for Wasserstein cost (remainder penalty)
...
if __name__ == "__main__":
    ...
    # Standard MWRL training (alpha=1.0)
    print("=== Standard MWRL Training ===")
    ...
    # Wasserstein-only training (alpha=0.0)
    print("\n=== Wasserstein-Only Training ===")
    model_w_only = create_mwrl_model(
        nesting_list=nesting_list,
        num_classes=100,
        pretrained=True,
        alpha=0.0,  # Turn off CE loss
        wasserstein_weight=0.1,
        beta=1.0,
        epsilon=0.5
    )
    
    loss, loss_dict = model_w_only(x, targets)
    print(f"Total loss: {loss.item():.4f}")
    print(f"CE loss (computed but not used): {loss_dict['ce_loss']:.4f}")
    print(f"Wasserstein loss: {loss_dict['wasserstein_loss']:.4f}")
